...
int max_current_gain_count = 20;
int max_blank_gain_count = 10;
...
int set_blank_buffer(int i_bpp, int rgain, int ggain, int bgain)
{
    int m = 0;
    int n = 0;
...
static int smi_thread(void *arg)
{
    ...
    for(;;){
        ...
        //if(set_current_gain_count > 0){
        if((set_current_gain_count < max_blank_gain_count) && (set_current_gain_count > 0)){
            log_debug("__________________set current gain, count : %d!______________\n", set_current_gain_count);       
...
static int video_thread(void *arg)
{
    VideoState *is = arg;
    AVFrame *frame = av_frame_alloc();
    AVFrame *frameRGB = NULL;//for RGB frame raw data, transfer from frame by swscale
    unsigned char *buffer = NULL;
    int numBytes;
    int swscale_pix_fmt = 0;
...
#if SMI_DECODE_MUTEX
    is->smi_decode_mutex = SDL_CreateMutex();
#endif
    ...
    if(!frameRGB){
        log_fatal("frameRGB alloc failed!\n");
        return AVERROR(ENOMEM);
    }
    ...
    for (;;) {
        ...
        if(set_current_gain_count > 0){
		    ...
		    if(set_current_gain_count > max_blank_gain_count){
                    	set_blank_buffer(icled_bits_per_pixel, red_current_gain, green_current_gain, blue_current_gain); 
		    }else{
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if ((ret = decoder_start(&is->viddec, video_thread, "video_decoder", is)) < 0)
            goto out;
        //add smi_transfer thread
        if ((ret = decoder_init(&is->smidec, avctx, &is->videoq, is->continue_read_thread)) < 0)
            goto fail;
        if ((ret = decoder_start(&is->smidec, smi_thread, "smi_transfer", is)) < 0)
            goto out;
...
int check_icled_current_gain_different(char *msgbuf){
    ...
    if((rgain != red_current_gain)||(ggain != green_current_gain)||(bgain != blue_current_gain)){   
        ...
        set_current_gain_count = max_current_gain_count;
    }
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    set_current_gain_count = max_current_gain_count;
    //set_current_gain_count = ;  //Jason @0620
